...
```typescript
// Get first page
const page1 = await client.tasks.getMessages(taskId, { page: 1, pageSize: 50 });

// Get next page if available
if (page1.pagination.has_more) {
  const page2 = await client.tasks.getMessages(taskId, { page: 2, pageSize: 50 });
}
```
...
```typescript
try {
  const message = await client.tasks.submitMessage(taskId, {
    content: userInput,
    actor: 'user'
  });
} catch (error) {
